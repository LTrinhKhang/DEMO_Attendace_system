...
#define CORRECT_PASSWORD "1234"  // Đổi mật khẩu đúng tại đây
...
{
    lv_obj_t * password_input = lv_event_get_target(e);
    const char * input_text = lv_textarea_get_text(password_input);

    if (strcmp(input_text, CORRECT_PASSWORD) == 0) {
        lv_label_set_text(ui_Label_Status, "Mật khẩu đúng!");
        lv_scr_load(ui_HomeScreen);
    } else {
        lv_label_set_text(ui_Label_Status, "Sai mật khẩu, thử lại!");
        lv_textarea_set_text(password_input, "");

        lv_anim_t anim;
        lv_anim_init(&anim);
        lv_anim_set_var(&anim, password_input);
        lv_anim_set_exec_cb(&anim, (lv_anim_exec_xcb_t)lv_obj_set_x);
        lv_anim_set_values(&anim, lv_obj_get_x(password_input) - 10, lv_obj_get_x(password_input) + 10);
...
void toggle_password_visibility(lv_event_t * e)
{
    ...
    visible = !visible;
    lv_textarea_set_password_mode(ui_txtPassword, !visible);
}
